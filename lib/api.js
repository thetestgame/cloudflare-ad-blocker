...
export const createZeroTrustListsOneByOne = async (items) => {
  ...
  for (let i = 0, listNumber = 1; i < items.length; i += LIST_ITEM_SIZE) {
    const chunk = items
      ...
      .map((item) => ({ value: item }));
    const listName = `CGAB List - Chunk ${listNumber}`;
...
export const createZeroTrustRule = (wirefilterExpression) =>
  requestGateway("/rules", {
    method: "POST",
    body: JSON.stringify({
      name: "CGAB Filter Lists",
      description:
        "Filter lists created by Cloudflare Gateway Ad Blocker Scripts. Avoid editing this rule. Changing the name of this rule will break the script.",
      enabled: true,
      action: "block",
      filters: ["dns"],
